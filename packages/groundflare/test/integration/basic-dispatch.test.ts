...
async function withWorkspace<T>(
  ...
  const extraDirs = config.services
    .filter(
      (s): s is typeof s & { kind: 'disk'; path: string } =>
...
describe('integration: single tenant with vars binding', () => {
  it('workerd accepts the config, router dispatches, env.GREETING reaches the handler', async () => {
    await withWorkspace(
      {
        manifest: {
          name: 'e2e',
...
describe('integration: two tenants, Host-based dispatch', () => {
  it('routes each Host header to the correct tenant', async () => {
    await withWorkspace(
      {
        manifest: {
          name: 'e2e',
  ...
  it('Host is case-insensitive', async () => {
    await withWorkspace(
      {
        manifest: {
          name: 'e2e',
...
describe('integration: service bindings between tenants', () => {
  it('worker A can fetch() worker B through an explicit service binding', async () => {
    await withWorkspace(
      {
        manifest: {
          name: 'e2e',
...
describe('integration: /__scheduled dispatch', () => {
  it("POST /__scheduled on localhost fires the target tenant's scheduled handler", async () => {
    await withWorkspace(
      {
        manifest: {
          name: 'e2e',
  ...
  it('rejects /__scheduled reached through a non-localhost Host', async () => {
    await withWorkspace(
      {
        manifest: {
          name: 'e2e',
...
describe('integration: same-worker Durable Object namespace', () => {
  it('env.COUNTER.get(id).fetch() reaches the DO class and ctx.storage persists across requests', async () => {
