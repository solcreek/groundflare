...
describe('Hono template — dual-target proof', () => {
  let tmp: string
  ...
  it('scaffolds expected files', async () => {
    const wrangler = await readFile(join(projectDir, 'wrangler.toml'), 'utf-8')
    expect(wrangler).toContain('name = "my-api"')
    expect(wrangler).toContain('compatibility_date = "2026-04-16"')

    const index = await readFile(join(projectDir, 'src/index.ts'), 'utf-8')
  ...
  it('bun analyze reports "ready" (no blockers)', async () => {
    const config = await readConfigFile(join(projectDir, 'wrangler.toml'))
    const fs: AnalyzeFs = {
  ...
  it('serves / and /health on real workerd (proof of CF compatibility)', async () => {
    const bundle = await bundleWorker({
      entry: join(projectDir, 'src/index.ts'),
    })
    ...
    const config = buildCapnpFromWorkspace(
      {
        name: 'hono-proof',
        workers: [{ name: 'my-api', domain: 'api.test', entryPath }],
      },
