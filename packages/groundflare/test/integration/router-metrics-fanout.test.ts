...
describe('integration: Router /__metrics fan-out to tenant shims', () => {
  it('scrape surfaces router series + KV + D1 binding series in one response', async () => {
  ...
  it('workers without shim bindings are skipped (no fan-out to raw user code)', async () => {
    const manifest: WorkspaceManifest = {
      name: 'e2e',
      workers: [
        {
