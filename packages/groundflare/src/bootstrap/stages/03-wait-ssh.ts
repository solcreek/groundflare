...
export interface WaitSshStageOptions {
  /** Total time to wait for SSH (TCP + handshake combined). Default 10 min. */
  ...
  return {
    ...
    async run(ctx) {
      ...
      await waitForSshTcpReady({
        ...
        port,
        maxWaitMs: opts.maxWaitMs ?? 10 * 60_000,
      ...
      // Brand new VPS — no reason to verify host keys (they were just
      // generated by cloud-init). `no` avoids mismatches when sshd
      // restarts mid-setup, and `/dev/null` prevents polluting the
      // operator's known_hosts with a transient key.
