...
export function generateBunShim(opts: BunShimOptions): string {
  ...
  return [
    ...
    '',
    '// ── binding facades — Phase 1 stubs ────────────────────────────',
    "// Replaced in Phase 2 (task #19) with bun:sqlite + S3 SDK wiring.",
    '// The facade surface must match the Cloudflare Workers types so user',
    '// code that already targets CF KV / D1 / R2 runs unchanged.',
    '',
    KV_STUB,
    '',
    D1_STUB,
...
const KV_STUB = `function makeKvFacade(binding, _shards) {
  const unimpl = (op) => () => {
    throw new Error(\`KV.\${op}() — groundflare Bun adapter not yet implemented (Phase 2). Binding: \${binding}\`)
  }
  return {
    get: unimpl("get"),
    getWithMetadata: unimpl("getWithMetadata"),
    put: unimpl("put"),
...
const D1_STUB = `function makeD1Facade(binding, _databaseName) {
  const unimpl = (op) => () => {
    throw new Error(\`D1.\${op}() — groundflare Bun adapter not yet implemented (Phase 2). Binding: \${binding}\`)
  }
  return {
    prepare: unimpl("prepare"),
    batch: unimpl("batch"),
    exec: unimpl("exec"),
    dump: unimpl("dump"),
  }
